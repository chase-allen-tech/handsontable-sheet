...
const CFroalaEditor = dynamic(() => import('./CFroalaEditor'), { ssr: false });


export default class TableLongEditor extends Handsontable.editors.BaseEditor {
    ...
    open() {
        ...
        ReactDOM.render(<CFroalaEditor row={this.curr_row} originalModel={this.originalValue} />, modal_editor);

        // Modal Actions
        span_close.onclick = () => { modal.style.display = 'none'; }
        btn_submit.onclick = (e) => {

            this.input_text = document.getElementById('long-text-' + this.curr_row).innerText;
...
export function TableLongRender(instance, td, row, col, prop, value, cellProperties) {
